                    ...
                    {upcomingEvents.map((event) => (
                        ...
                        >
                            <div className="relative aspect-[3/4] bg-gray-200 mb-4 overflow-hidden">
                                ...
                                <div className={`absolute bottom-0 left-0 right-0 p-1.5 md:p-4 text-white ${event.gallery && event.gallery.length > 0 ? 'bg-accent animate-pulse-slow' : 'bg-black/80'}`}>
                                    <div className={`font-mono text-[8px] md:text-xs mb-0.5 md:mb-1 ${event.gallery && event.gallery.length > 0 ? 'text-white/90' : 'text-accent'}`}>
                                        {formatDate(event.date)} {event.gallery && event.gallery.length > 0 && `• 📷 ${event.gallery.length}`}
                                    </div>
                                    <div className="font-oswald text-xs md:text-2xl font-bold uppercase leading-tight">
                                        {event.city}
                                    </div>
                                    <div className={`font-mono text-[8px] md:text-xs mt-0.5 md:mt-1 ${event.gallery && event.gallery.length > 0 ? 'text-white/80' : 'text-gray-300'}`}>
                                            ...
                                            {yearEvents.map((event) => (
                                                ...
                                                >
                                                    <div className="relative aspect-[3/4] bg-gray-200 mb-4 overflow-hidden">
                                                        ...
                                                        <div className={`absolute bottom-0 left-0 right-0 p-1.5 md:p-4 text-white ${event.gallery && event.gallery.length > 0 ? 'bg-accent animate-pulse-slow' : 'bg-black/80'}`}>
                                                            <div className={`font-mono text-[8px] md:text-xs mb-0.5 md:mb-1 ${event.gallery && event.gallery.length > 0 ? 'text-white/90' : 'text-gray-400'}`}>
                                                                {formatDate(event.date)} {event.gallery && event.gallery.length > 0 && `• 📷 ${event.gallery.length}`}
                                                            </div>
                                                            <div className="font-oswald text-xs md:text-2xl font-bold uppercase leading-tight">
                                                                {event.city}
