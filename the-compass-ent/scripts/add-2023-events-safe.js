...
// 이 패턴을 찾되, 가장 마지막 ]; 를 찾아야 함 (companyEvents 등 다른 배열이 뒤에 있을 수 있으므로 주의)
// 하지만 events 배열은 파일 상단에 정의되어 있음.

// events 배열 시작 위치 찾기
const startIdx = content.indexOf('export const events: Event[] = [');
if (startIdx === -1) {
...
// 삽입
// endIdx는 ']'의 위치. 그 앞에 쉼표와 새 이벤트를 넣어야 함.
// 기존 마지막 요소 뒤에 쉼표가 있는지 확인
const beforeEnd = content.substring(startIdx, endIdx);
const hasTrailingComma = beforeEnd.trim().endsWith(',');
...
const newContent = content.slice(0, endIdx) + insertStr + content.slice(endIdx);
